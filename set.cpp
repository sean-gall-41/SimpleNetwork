...
/* PRIVATE FUNCTIONS */
static uint64_t __default_hash(const uint32_t key);
static int __get_index(SimpleSet *set, const uint32_t key, uint64_t hash, uint64_t *index);
static int __assign_node(SimpleSet *set, const uint32_t key, uint64_t hash, uint64_t index);
static void __free_index(SimpleSet *set, uint64_t index);
static int __set_contains(SimpleSet *set, const uint32_t key, uint64_t hash);
static int __set_add(SimpleSet *set, const uint32_t key, uint64_t hash);
...
uint32_t* set_to_array(SimpleSet *set, uint64_t *size) {
    *size = set->used_nodes;
    uint32_t* results = (uint32_t*)calloc(set->used_nodes + 1, sizeof(uint32_t));
    uint64_t i, j = 0;
    size_t len;
    for (i = 0; i < set->number_nodes; ++i) {
        if (set->nodes[i] != NULL) {
...
static uint64_t __default_hash(const uint32_t key) {
	...
	uint8_t *key_bytes = (uint8_t *)calloc(1, len);
	memcpy(key_bytes, &key, len);
...
static int __get_index(SimpleSet *set, const uint32_t key, uint64_t hash, uint64_t *index) {
    ...
    while (1) {
        ...
        } else if (hash == set->nodes[i]->_hash && key == set->nodes[i]->_key) {
            *index = i;
            return SET_TRUE;
        }
        ++i;
